...
class ZTRadarCollector(BaseCollector):
    """涨停雷达采集器"""

    def __init__(self, **kwargs):
        super().__init__(source_name="zt_radar", **kwargs)
        self.data_source = TonghuashunSource()

    def collect(self, limit: int = 20, **kwargs) -> List[Event]:
        """采集涨停雷达数据"""
        events = []
        items = self.data_source.get_zt_radar_list(limit)
